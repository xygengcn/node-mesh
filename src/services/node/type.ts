 ...
 * 函数返回内容
 */
export type NodeActionResult<F extends NodeAction, K extends NodeActionKey<F>> = K extends NodeActionKey<F>
    ? ReturnType<NodeActionFunction<F, K>>
    : Promise<any>;
...
export type NodeActionFunctionParam<F extends NodeAction, K extends NodeActionKey<F>> = K extends NodeActionKey<F>
    ? Parameters<NodeActionFunction<F, K>>
    : never;
...
export type NodeActionPromise<F extends NodeAction> = {
    [K in NodeActionKey<F>]: NodeActionResult<F, K> extends Promise<any>
        ? NodeActionFunction<F, K>
        : (...args: NodeActionFunctionParam<F, K>) => Promise<NodeActionResult<F, K>>;
};

/**
 * 节点的emit key
 */
export type NodeEmitKey<T extends string, K extends NodeAction> = T extends
    | keyof K
    | `${'request' | 'subscribe' | 'socket' | 'emitter' | 'node'}:${string}`
    | 'emitter:logger'
    ? never
    : T;
...
export type NodeOnListener<T extends string, K extends NodeAction, F extends NodeAction> = T extends 'emitter:logger'
    ? (level: EmitterDebugLevel, title: string, ...args: any[]) => void
